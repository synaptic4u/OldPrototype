...
class DB
{
    ...
    // Database table cleaning - Cleans everything older than 60 minutes.
    public function cleanStream()
    {
        $sql = 'delete 
				  from stream 
				 where (UNIX_TIMESTAMP(now())-UNIX_TIMESTAMP(DatedOn)) > 3600 
				   and 1 = ?';

        $result = $this->query([1], $sql);
    }

    public function cleanMyKeys()
    {
        $sql = 'delete 
				  from mykeys 
				 where (UNIX_TIMESTAMP(now())-UNIX_TIMESTAMP(DatedOn)) > 3600 
